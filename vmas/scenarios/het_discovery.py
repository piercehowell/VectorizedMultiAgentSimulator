...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.n_targets = kwargs.get("n_targets", 5)
        self._min_dist_between_entities = kwargs.get("min_dist_between_entities", 0.2)
        self._lidar_range = kwargs.get("lidar_range", 0.35) # sensing range for avoidance
        ...
        # agent specifications; default 3 agents
        self.agent_specs = kwargs.get("agent_specs",
                                      {
                                          'name': 'default_agents',
                                          'agents':
                                          [
                                              {'id': "001", 'radius':0.1, 'speed_multiplier':[1, 0.01]},
                                              {'id': "010", 'radius':0.05,'speed_multiplier': [1.5, 0.01]},
                                              {'id': "001", 'radius':0.15,'speed_multiplier': [0.5, 0.01]},
                                          ]
                                      }
                                      )
        
        for i, agent_spec in enumerate(self.agent_specs['agents']):
            
            agent_id = agent_spec['id']
            agent_radius = agent_spec['radius']
            agent_speed_multiplier = agent_spec['speed_multiplier']
            ...
            agent = Agent(
                ...
                render_action=True,
                u_range=[1, 1],
                u_multiplier=agent_speed_multiplier,
...
if __name__ == "__main__":
    render_interactively(__file__, control_two_agents=True, save_render=True)
